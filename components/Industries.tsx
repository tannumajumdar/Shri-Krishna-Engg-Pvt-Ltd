 ...
 *
 * The record stores short internal names ("Power", "Automotive"); the sector
 * names a visitor recognises are longer, so they are mapped here rather than
 * rewritten in the database, which the products API also reads.
 */
const SECTOR_NAMES: Record<string, string> = {
  Power: "Power & Energy",
  Infrastructure: "Industrial Infrastructure",
  Construction: "Steel & Heavy Engineering",
  Manufacturing: "Manufacturing",
  Automotive: "Mining & Minerals",
  Electrical: "Aluminium & Metals",
};

export function Industries({ items }: { items?: Industry[] } = {}) {
  const industries = (items?.length ? items : staticIndustries).slice(0, 6);
      ...
      <div className="container">
        ...
        >
          {industries.map((industry, i) => {
            const label = SECTOR_NAMES[industry.name] ?? industry.name;
            return (
