...
import { socialLinkCreateSchema } from "@/lib/validations";
...
export const POST = handle(async (req: NextRequest) => {
  ...
  const body = await parseBody(req, socialLinkCreateSchema);
  const row = await prisma.socialLink.create({ data: body });
  return ok(row, 201);
});
