...
import { statisticCreateSchema } from "@/lib/validations";
...
export const POST = handle(async (req: NextRequest) => {
  ...
  const body = await parseBody(req, statisticCreateSchema);
  const row = await prisma.statistic.create({ data: body });
  return ok(row, 201);
});
