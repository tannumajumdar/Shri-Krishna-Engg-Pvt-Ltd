...
import { qualityPointUpdateSchema } from "@/lib/validations";

export const PUT = handle(async (req: NextRequest, ctx: RouteCtx) => {
  await requireAuth(req);
  const id = await numericParam(ctx);
  const body = await parseBody(req, qualityPointUpdateSchema);
  const row = await prisma.qualityPoint.update({ where: { id }, data: body });
  return ok(row);
});
...
export const DELETE = handle(async (req: NextRequest, ctx: RouteCtx) => {
  await requireAuth(req);
  const id = await numericParam(ctx);
  await prisma.qualityPoint.delete({ where: { id } });
